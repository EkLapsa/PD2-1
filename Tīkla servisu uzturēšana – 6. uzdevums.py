#Tīkla servisu uzturēšana – 6. uzdevums
nodes = [
...
def serv_stavoklis(nodes):
    vard = {}

    for name in nodes:
        namer, ip, latente, stavoklis = name.split(";")
        if namer.startswith("Srv"):
            vard[ip] = stavoklis.upper()
    return vard
print(serv_stavoklis(nodes))
